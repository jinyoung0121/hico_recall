...
class HICOEvaluator():
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat):
        ...
        prediction_file='prediction_file.json'
        gt_file='gt_file.json'
        image_per_triplet_prediction='image_per_triplet_prediction.json'
        image_per_gt='image_per_gt.json'
        triplet_prediction_per_image='triplet_prediction_per_image.json'
        gt_triplet='gt_triplet.json'
        prediction_result={}
        gt_result={}
        image_per_triplet_prediction_result={}
        image_per_gt_result={}
        triplet_prediction_per_image_result={}
        triplet_prediction_per_image_result_final={}
        gt_triplet_result={}
        self.preds = []
        for index, img_preds in enumerate(preds):
            # len(preds)=9546
            # preds[0]=dict(['labels':,'boxes':,'verb_scores':,'sub_ids':,'obj_ids':])
            ...
            hoi_scores = img_preds['verb_scores'] # shape : (100,117)
            verb_labels = np.tile(np.arange(hoi_scores.shape[1]), (hoi_scores.shape[0], 1)) # shape : (100,117)
            subject_ids = np.tile(img_preds['sub_ids'], (hoi_scores.shape[1], 1)).T # shape : (100,117)
            object_ids = np.tile(img_preds['obj_ids'], (hoi_scores.shape[1], 1)).T # shape : (100,117)
            ...
            pdb.set_trace()
        # ---make image_per_triplet_prediction.json---

            # pred_img_name='img_'+str(index)
            # for hoi in hois:
            #     category=f'({int(bboxes[hoi["subject_id"]]["category_id"])}, {int(bboxes[hoi["object_id"]]["category_id"])}, {int(hoi["category_id"])})'
            #     if category not in image_per_triplet_prediction_result:
            #         image_per_triplet_prediction_result[category]=[(pred_img_name,float(hoi['score']))]
            #     else:
            #         image_per_triplet_prediction_result[category].append((pred_img_name,float(hoi['score'])))
        
        # ---make triplet_prediction_per_image.json---

            # pred_image_name='img_'+str(index)
            # for hoi in hois:
            #     category=f'({int(bboxes[hoi["subject_id"]]["category_id"])}, {int(bboxes[hoi["object_id"]]["category_id"])}, {int(hoi["category_id"])})'
            #     if pred_image_name not in triplet_prediction_per_image_result:
            #         triplet_prediction_per_image_result[pred_image_name]=[(category,float(hoi['score']))]
            #         triplet_prediction_per_image_result_final[pred_image_name]=[]
            #     else:
            #         triplet_prediction_per_image_result[pred_image_name].append((category,float(hoi['score'])))
        
        # ---make image_per_triplet_prediction.json---

        # image_per_triplet_prediction_result=dict(sorted(image_per_triplet_prediction_result.items(),key=lambda x:x[0]))
        # for key in image_per_triplet_prediction_result:
        #     image_per_triplet_prediction_result[key]=sorted(image_per_triplet_prediction_result[key],key=lambda x:-x[1])

        # with open(image_per_triplet_prediction,'a') as f:
        #     json.dump(image_per_triplet_prediction_result,f,indent='\t')

        # ---make triplet_prediction_per_image.json---

        # triplet_prediction_per_image_result=dict(sorted(triplet_prediction_per_image_result.items(),key=lambda x:x[0]))
        # for key in triplet_prediction_per_image_result:
        #     triplet_prediction_per_image_result[key]=sorted(triplet_prediction_per_image_result[key],key=lambda x:-x[1])
        
        # for key in triplet_prediction_per_image_result:
        #     for value in triplet_prediction_per_image_result[key]:
        #         triplet_prediction_per_image_result_final[key].append(value[0])
        
        # with open(triplet_prediction_per_image,'a') as f:
        #     json.dump(triplet_prediction_per_image_result_final,f,indent='\t')


        self.gts = []
        for index,img_gts in enumerate(gts):
            ...
            for hoi in self.gts[-1]['hoi_annotation']:
                ...
                # gt_img_name='img_'+str(index)
                # gt_triplet=f'({int(triplet[0])}, {int(triplet[1])}, {int(triplet[2]))})'
                # if img_name not in gt_result:
                #     gt_result[gt_img_name]=[gt_triplet]
                # else:
                #     if gt_triplet not in gt_result[gt_img_name]:
                #         gt_result[gt_img_name].append(gt_triplet)


        # ---make image_per_gt.json---

        #         gt_img_name='img_'+str(index)
        #         gt_triplet=f'({int(triplet[0])}, {int(triplet[1])}, {int(triplet[2])})'
        #         if gt_triplet not in image_per_gt_result:
        #             image_per_gt_result[gt_triplet]=[gt_img_name]
        #         else:
        #             image_per_gt_result[gt_triplet].append(gt_img_name)
        # image_per_gt_result=dict(sorted(image_per_gt_result.items(),key=lambda x:x[0]))

        # with open(image_per_gt,'a') as f:
        #     json.dump(image_per_gt_result,f,indent='\t')


        # ---make gt_triplet.json---

        # for triplet in self.gt_triplets:
        #     gt_triplet_result[str(triplet)]=int(self.sum_gts[triplet])
        # gt_triplet_result=dict(sorted(gt_triplet_result.items(),key=lambda x:x[0]))

        # with open(gt_triplet,'a') as f:
        #     json.dump(gt_triplet_result,f,indent='\t')

        # with open(gt_file,'a') as f:
        #     json.dump(gt_result,f,indent='\t')
